...
public class Codegen {
    ...
    public static void loadSymAddressIntoT0(IdNode var) {
        if (var.sym().isLocal) {
            Codegen.generateIndexed("la", Codegen.T0, Codegen.FP, var.sym().localOffset, "Load address of " + var.name());
        } else {
            Codegen.generateWithComment("la", "Load address of " + var.name(), Codegen.T0, var.sym().globalLabel);
        }
    }

    public static void loadSymValIntoT0(IdNode var) {
        if (var.sym().isLocal) {
            Codegen.generateIndexed("lw", Codegen.T0, Codegen.FP, var.sym().localOffset, "Load value of " + var.name());
        } else {
            Codegen.generateWithComment("lw", "Load value of " + var.name(), Codegen.T0, var.sym().globalLabel);
        }
    }
}
